...
public class ActivityTask : MonoBehaviour
{
    ...
    /// <param name="activity">istanza del CharacterActivity a cui il ActivityPoint si assocer�</param>
    /// <returns></returns>
    ...
    /// Se lo stato di allerta dell'nPCBehaviour che esegue il task � != da CharacterAlertState.Unalert allora
    ...
    public async Task executeTask(CharacterManager character, BaseNPCBehaviourManager nPCBehaviour, CharacterMovement characterMovement, Action executeDuringTask = null) {
        ...
        while (Time.time < end) {
            ...
            // Interrompi se il Play è stato fermato o il task è stato distrutto:
            ...
            // interrompi il task se lo stato di allerta non � pi� [CharacterAlertState.Unalert]
    ...
    public Vector2 getTaskDirection() {
       // guardia difensiva: se il task è stato distrutto (es. uscita dal Play) non accedere a gameObject
       if (this == null) {
           return Vector2.zero;
       }
       return new Vector2(
    ...
    void OnDrawGizmos() {
        ...
        // tra la camera della scena e l'oggetto � <10
